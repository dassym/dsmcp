...
class BaseMainWindow(QMainWindow):
    ...
    def onWorkspaceChanged(self, workspace):
        self._labelStatusWorkspace.setText(str(workspace.name))
        for dlg in self.dialogs.values():
    ...
    def onRegistersView(self, checked=None):
        ...
        self.dialogs['registersview'].exec()
...
class BaseQtApp(BaseApp, QApplication):
    ...
    def getAvailableActions(self, context={}):
        ...
        :rtype: Set of str 
        '''
        ret = set(['actionQuit', 'actionLink', 'actionConnect', 'actionHelp','actionAbout', 'actionRegistersView','actionAutoRefresh'])
    ...
    def errorInfo(self):
        ret = super().errorInfo()
        ret.update({'Qt':QT_VERSION_STR,'PyQt':PYQT_VERSION_STR})
        return ret
