...
class QSystemMode(QFrame):
    ...
    def refresh(self):
        self._labelText.setVisible(self.dev or self.parent().app.isInDevMode())
    ...
    def onConfigStateChanged(self, config=None, checked=None):
        self.parent().log.debug(f'onConfigStateChanged({config.name!s}, {checked!s})')
        self.parent().log.debug("{r.name:s} = 0x{r.value:04x} ({r.value:d})".format(r=self._board.regs.smr))
