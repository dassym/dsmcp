...
class QRegisterFlag(QFrame):
    ...
    def _initialize(self):
        for i, flag in enumerate(self._reg):
            o = QCheckBox("{0:d}:{1!s}".format(i,flag.name), self)
            o.setToolTip(flag.descr)
            o.setObjectName(self._reg.name+'CheckBox'+flag.name)
            self._widgets[flag.name] = o
            self._glayout.addWidget( o, i // self._cols, i % self._cols, 1, 1)
...
class QRegisterFlagDev(QRegisterFlag):
    ...
    def refresh(self):
        if self._reg is None: return
        self._labelText.setText("{r.name:s} = 0x{r.value:04x} ({r.value:d})".format(r=self._reg))
