...
@author: fv
'''
from PyQt5.Qt import QWizard, QCoreApplication, QDate
...
class QWizardProdStartup(WizardProdStartup, QWizard, Ui_WizardProdStartup): 
    ...
    def prepare(self):
        ...
        if self.config.customer:
            self.setCustomer(self.db.getCustomer(self.config.customer,insensitive=True))
            start = 1
            #self.setField('customer', self.db.getCustomer(self.config.customer))
        if self.config.board_type:
            board_variant = self.db.getBoardVariant(self.config.board_type,insensitive=True)
            self.setBoard(board_variant.board)
    ...
    def setBoard(self, board):
        self.log.debug('setBoard({0!s})'.format(board))
        self.wizardPageBoard.initializePage()
        self.comboBoxBoard.setCurrentIndex(self.comboBoxBoard.findData(board))
        #self.wizardPageVariant.initializePage()
    ...
    def setDate(self, date):
        self.log.debug('setDate({0!s})'.format(date))
        self.dateEditManufacturingDate.setDate(QDate(date))
    ...
    def getBoard(self):
        return self.comboBoxBoard.itemData(self.field('board'))
    ...
    def getDate(self):
        return self.field('date').toPyDateTime().date()
