...
class BasicWindow(BaseMainWindow, Ui_BasicWindow):
    ...
    def doRebootBoard(self, checked=None):
        ...
        self.board.reboot()
        self.app.stopComm()
        self.log.info('Wait before reconnection ({}s)'.format(self.board.wait_after_reboot))
        self.app.sleep(self.app.board.wait_after_reboot)
    ...
    def doSendFirmware(self, checked=None):
        ...
        if dlg.exec() == QDialog.Accepted:
            ...
            self.board.flashFirm(str(dlg.getFirmware().fpath), self._flasProgressDialog.progress)
    ...
    def getAvailableActions(self, context={}):
        ...
        if self.board is not None: 
            if self.isConnected():
                ...
                ret |= set(('actionMotorMode', 'actionRebootBoard'))
                if self.board.isOnStandby():
