...
class BaseApp(TellerInterface, object):
    ...
    '''Application default board access level. :class:`dapi2.DApiAccessLevel` ''' 
    ...
    def prepareCfg(self, parser, preconf):
        ...
        dcom_grp.add_argument("-S", "--serial", dest="serial", nargs='?', metavar="SERIAL", const='default',
    ...
    def startUp(self):
        ...
        Actions performed:
            ...
            - Application environement checking
    ...
    def connectBoard(self, level=None):
        ...
        if self._board is not None:
            ...
            self._board.getRegisters('scsr', undefined_only=True)
            ...
            self._board.refreshAll(undefined_only=True)
            t.stop()
    ...
    def handleError(self, exception):
        ...
        txt += '\n-------\n'+'\n'.join( "{}: {}".format(k,v) for k,v in self.errorInfo().items())
        txt += '\n-------\n'
        self.log.error(txt)
        self.terminate(1)
