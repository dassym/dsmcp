...
from _version import __version__, DATE
from app.common import DEFAULT_LOG_DIR, LOG_FORMAT, USR_FIRM_DIR, \
    USR_DATA_APP_DIR, USR_DATA_DIR, DEFAULT_USR_CONFIG, DEFAULT_CCONFIG_NAME, OS_DEFAULT_CCONFIG_NAME
from app.teller import TellerInterface
...
class Launcher(TellerInterface):
    ...
    def __init__(self, app_dir, add_help=False):
        ...
        if not os.path.exists(DEFAULT_USR_CONFIG):
            try:
                copyfile(os.path.join(app_dir, OS_DEFAULT_CCONFIG_NAME)  , DEFAULT_USR_CONFIG)
            except OSError:
                self.sayMedium("Copying of the default ini file to `{0!s}` failed!".format(DEFAULT_USR_CONFIG))
            else:
...
""".format(dy=DATE.strftime('%Y'))
        ...
        gui_grp.add_argument("--cli", dest="gui", action='store_false', default=False, help=BaseApp.tr("If present, launches the application without the graphical user interface (default)"))
        gui_grp.add_argument("--gui", dest="gui", action='store_true', default=False, help=BaseApp.tr("If present, launches the application with the graphical user interface (default:witout GUI)"))
